...
# --- 脱水提示词：指导廉价 LLM 压缩信息 ---
DEHYDRATE_PROMPT = """你是一个记忆压缩器。请将以下内容压缩为一段简洁但有温度的叙述。

压缩规则：
1. 用第三人称叙述，像在回忆一件事，不要用"用户"这种称呼，用原文中的名字或代称
2. 保留关键原话（用引号标注），尤其是有情绪、有个性、有画面感的表达
3. 保留情绪脉络和态度变化，不要只留结论
4. 关键数字、日期、名称必须保留
5. 去除重复、冗余、纯寒暄，但不要把对话压成数据库条目
6. 如有待办事项或未完成的计划，在末尾用"待办："标注
7. 目标压缩率 > 60%，但可读性优先于压缩率
8. 输出纯文本，不要用 JSON、列表或任何结构化格式"""
DIGEST_PROMPT = """你是一个日记整理专家。用户会发送一段包含今天各种事情的文本（可能很杂乱），请你将其拆分成多个独立的记忆条目。

整理规则：
1. 每个条目应该是一个独立的主题/事件（不要混在一起）
2. content 用第三人称叙述，像在回忆一件事，不要用"用户"这种称呼，用原文中的名字或代称
3. 为每个条目自动分析元数据
4. 去除纯重复和无信息量的内容，但保留有情绪、有个性、有画面感的表达和关键原话（用引号标注）
5. 同一主题的零散信息应合并为一个条目
6. 如果有待办事项，单独提取为一个条目
7. 单个条目内容不少于50字，过短的零碎信息合并到最相关的条目中
8. 总条目数控制在 2~8 个，避免过度碎片化
9. 在 content 中对人名、地名、专有名词用 [[双链]] 标记（如 [[婷易]]、[[Obsidian]]），普通词汇不要加
